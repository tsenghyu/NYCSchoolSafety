...
#loading data 
school_clean <- read.csv("/Users/tomyu/Desktop/school_clean.csv")
...
```{r}
cleandata_5 <- school_clean %>%
  select(School.Year, Borough, Major.N, Oth.N, NoCrim.N, Prop.N, Vio.N) %>%
  group_by(School.Year, Borough) %>%
  summarise(
    Major.N = sum(Major.N, na.rm = TRUE),
    Oth.N = sum(Oth.N, na.rm = TRUE),
    NoCrim.N = sum(NoCrim.N, na.rm = TRUE),
    Prop.N = sum(Prop.N, na.rm = TRUE),
    Vio.N = sum(Vio.N, na.rm = TRUE)
  ) %>%
  ungroup()

cleandata_5 <- cleandata_5 %>%
  pivot_longer(cols = -c(School.Year, Borough), names_to = "crime", values_to = "count")
...
cleandata_8 <- cleandata_8 %>%
  pivot_longer(cols = -Geographical.District.Code, names_to = "crime", values_to = "count")

# Ensure that the count column is numeric and filter out non-numeric values
cleandata_8 <- cleandata_8 %>%
  mutate(count = as.numeric(count)) %>%
  filter(!is.na(count))

cleandata_8 <- cleandata_8 %>%
  group_by(Geographical.District.Code, crime) %>%
...
```{r}
# Summarize the data for PCA analysis
cleandata_10 <- school_clean %>%
  filter(School.Year == '2013-14') %>%
  mutate(Register = as.numeric(gsub(",", "", Register))) %>%
  group_by(Geographical.District.Code) %>%
  summarise(
    Major.N = sum(Major.N, na.rm = TRUE),
    Oth.N = sum(Oth.N, na.rm = TRUE),
    NoCrim.N = sum(NoCrim.N, na.rm = TRUE),
    Prop.N = sum(Prop.N, na.rm = TRUE),
    Vio.N = sum(Vio.N, na.rm = TRUE),
    X..Schools = sum(X..Schools, na.rm = TRUE),
    Register = sum(Register, na.rm = TRUE)
  ) %>%
  ungroup() %>%
  filter(Geographical.District.Code != 0)
